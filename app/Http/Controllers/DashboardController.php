...
class DashboardController extends AccountBaseController
{
    ...
    {
        $project_status = Project::select('projects.pm_id', 'users.name as manager_name', 'projects.client_id', 'clients.name as client_name', 'projects.id', 'projects.project_name', 'projects.project_budget', 'pm_projects.created_at as project_start')
        ...
        foreach ($project_status as $project) {
           ...
           $events = [];
            ...
            $project_deliverable=Project::join('contract_signs', 'contract_signs.project_id', '=', 'projects.id')
                ->where('projects.status', $project_id)
                ->count();

            $tasks_proper_assigned= DB::table('project_milestones')
            ...
            $deliverable_signed_days = Carbon::parse($project->project_start)->addHours(72);
                      
            if($deliverable_signed_days<= $current_time && $project_deliverable==0 && $tasks_proper_assigned ==0){
            ...
            }else if($deliverable_signed_days <= $current_time && $project_deliverable == 1 && $tasks_proper_assigned == 0){
                $events[] = (object) ['message' => 'Deliverable is not signed but tasks are assigned properly', 'date' => $deliverable_signed_days];
            }else if($deliverable_signed_days <= $current_time && $project_deliverable == 1 && $tasks_proper_assigned == 1){
        ...
        return view('dashboard.admin',compact('project_status'));
